...
function renderSubmissions() {
  ...
  displayedSubmissions.forEach((submission) => {
    ...
    if (!submission.fields.Screenshot || submission.fields.Screenshot.length === 0) {
      ...
      photoUrl = submission.fields.Screenshot[0].thumbnails?.large?.url || submission.fields.Screenshot[0].url || "https://hc-cdn.hel1.your-objectstorage.com/s/v3/ee0109f20430335ebb5cd3297a973ce244ed01cf_depositphotos_247872612-stock-illustration-no-image-available-icon-vector.jpg";
    }
        ...
        <div class="links">
          ${
            submission.fields["Code URL"]
              ? `<a href="${submission.fields["Code URL"]}" class="github-button" target="_blank"><i class="fa-brands fa-github"></i> Github</a>` : ""
          }
          ${
            submission.fields["Playable URL"]
              ? `<a href="${submission.fields["Playable URL"]}" class="demo-button" target="_blank"><i class="fa-solid fa-link"></i> Demo</a>` : ""
          }
        </div>
      </div>
    `;
